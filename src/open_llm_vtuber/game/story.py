...
class StoryScene(BaseModel):
    scene_id: str
    character_id: str  # 对应conf.yaml中的character配置
...
class GameManager:
    def __init__(self, service_context):
        ...
        self.websocket = None  # 存储WebSocket连接
        
    def set_websocket(self, websocket):
        """设置当前WebSocket连接"""
        self.websocket = websocket
        
    async def send_debug_info(self, debug_info: dict):
        """发送调试信息到前端"""
        if self.websocket:
            try:
                await self.websocket.send_json({"debug_info": debug_info})
        ...
        try:
            ...
            for scene_id, scene_data in story_data_dict.get('scenes', {}).items():
                ...
                scenes[scene_id] = StoryScene(
                    scene_id=scene_id,
                    character_id=scene_data['character_id'],
                    background_image=scene_data['background_image'],
    ...
    def get_current_scene_data(self) -> Dict:
        ...
        response_data = {
            ...
            "use_live2d": True  # 告诉前端使用Live2D模型
        }
    ...
    def process_user_choice(self, choice_id: str) -> Dict:
        ...
        if new_scene:
            self.game_state.dialogue_history.append({
                "speaker": "character",
                "character_id": new_scene.character_id,
...
"""
        ...
        try:
            ...
            # 如果有WebSocket连接，发送调试信息
            # 注意：这里假设有一个方法可以访问当前的WebSocket连接
            try:
                if hasattr(self, "send_debug_info") and callable(self.send_debug_info):
                    await self.send_debug_info(debug_info)
            except Exception as e:
